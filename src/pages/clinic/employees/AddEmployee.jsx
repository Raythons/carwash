...
import { useCreateEmployee, useVacationTypes } from '../../../hooks/queries/useEmployeeQueries';
import { ArrowLeft, Save, User, Mail, Phone, Building2, DollarSign, Calendar } from 'lucide-react';
...
export default function AddEmployee() {
  ...
  const validateForm = () => {
    ...
    if (!formData.password.trim()) {
      ...
      newErrors.password = t("employees.validation.password_min_length");
    }
      ...
      <div className="flex items-center justify-between">
        <div className={cn("flex items-center space-x-4", isRTL && "space-x-reverse")}>
          ...
          <div className={isRTL ? "text-right" : "text-left"}>
            <h1 className="text-2xl font-bold text-gray-900">{t("employees.add_new")}</h1>
            <p className="text-gray-600">{t("employees.add_desc")}</p>
          </div>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              />
              {errors.password && (
                <p className={cn("mt-1 text-sm text-red-600", isRTL ? "text-right" : "text-left")}>{errors.password}</p>
              )}
            </div>
            ...
            >
              {createEmployeeMutation.isPending ? (
                ...
                <div className={cn("flex items-center space-x-2", isRTL && "space-x-reverse")}>
                  ...
                  <span>{t("employees.save_employee")}</span>
                </div>
              )}
            </Button>
          </div>
