...
import { Package, BarChart3, ArrowLeft, CalendarIcon } from "lucide-react"
...
export function TopSellingProducts() {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading, refetch } = useInfiniteQuery({
    queryKey: ["top-selling-products", timeRange, customStartDate, customEndDate],
    queryFn: ({ pageParam = 1 }) =>
      storageApi.getTopSellingProducts(pageParam, 20, timeRange, customStartDate, customEndDate),
        ...
        <CardContent>
          {allProducts.length > 0 ? (
            <div className="space-y-3">
              {allProducts.map((product, index) => (
                <div
                  ...
                  className="flex items-center gap-4 p-4 bg-gray-50 dark:bg-zinc-800 rounded-lg hover:bg-gray-100 dark:hover:bg-zinc-700 transition-colors"
                >
                  <div className="flex-shrink-0 w-10 h-10 rounded-full bg-primary-100 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400 flex items-center justify-center font-bold text-lg">
                    {index + 1}
                  </div>
                  ...
                  <div className="text-right">
                    <p className="font-bold text-primary-600 dark:text-primary-400 text-xl">
                      {formatNumberWithThousands(product.totalSold)}
                    </p>
                    <p className="text-xs text-gray-600 dark:text-zinc-400">وحدة مباعة</p>
                  </div>
                </div>
              ))}
            ...
            <div className="text-center py-12 text-gray-500 dark:text-zinc-400">
              <Package className="w-16 h-16 mx-auto mb-4 text-gray-400 dark:text-zinc-600" />
              <p className="text-lg">لا توجد بيانات مبيعات حتى الآن</p>
            </div>
          )}
        </CardContent>
      </Card>
